...
//		document.getElementById('totalRewardDisplay').innerHTML = document.getElementById(rewardArray[0]+"_no").checked;
		chenged = false;
...
//			document.getElementById(rewardArray[i]+"_money_id").value = 0;
		...
		if(document.getElementById(rewardArray[i]+"_travel").checked){
			var travel = document.getElementById(rewardArray[i]+"_travel_id").value;
			if((travel != null) && isNaN(travel)){
				//数値でない場合にtrue
				var num = rewardArray[i].slice(-1);//最後の1文字を切り出す
				alert(num+"番目の講師の謝礼情報に数値以外が入力されています");
				return false;
